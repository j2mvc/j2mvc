		...
		var methods = {
				...
				create:function(){
					...
					if(tabOffset == 'top' ){// 标签栏偏移顶部			
						...
						_tabsOuter.addClass('offset-top');
						...
						_panel.append(_tabsOuter);
						_panel.append(_contentsOuter);
						_tabsOuter.addClass('offset-left');
						...
						_panel.append(_contentsOuter);
						_panel.append(_tabsOuter);		
						_tabsOuter.addClass('offset-right');
					}else if(tabOffset == 'bottom'){// 标签栏偏移底部
						_panel.append(_contentsOuter);
						_panel.append($('<div style="clear:both"></div>'));
						_panel.append(_tabsOuter);
						_tabsOuter.addClass('offset-bottom');
					}
					this.createHeader();
					this.createFooter();
...
//						}
				...
				_resize:function(){
					...
					_tabsOuter.find('.tab:last').addClass('last');
					if( tsw && tsw == 'match_parent' ){
						var tabs = _tabsOuter.find('.tab');
						if(tos == 'top' || tos == 'bottom'){// 标签栏偏移顶部或底部
							tabs.width((_tabsOuter.width() - tabs.size() ) /tabs.size());
							tabs.height(_tabsOuter.height());
						}
					}
					this.touchTabs();
				},
				...
				touchTabs:function(){
					var _this = this;
					var tos = this.tabLayout.offset;
					if((tos == 'top' || tos == 'bottom') && 
							(_tabs.width() - _tabsOuter.width() < 0)){
						// tab总宽度小于屏幕宽度，不滑动
						return;
					} else if((tos == 'left' || tos == 'right') && 
							(_tabs.height() - _tabsOuter.height() < 0)){
						// tab总宽度小于屏幕宽度，不滑动
						return;
					}
					//滑动范围在5x5内则做点击处理，s是开始，e是结束
				    var init = {sx:0,ex:0,sy:0,ey:0};
				    // 开始手势
				    _tabs[0].addEventListener("touchstart",function(event){
				        init.sx = event.targetTouches[0].clientX;
				        init.sy = event.targetTouches[0].clientY;
				    }, false);
				    _tabs[0].addEventListener("touchmove",function(event) {
				        event.preventDefault();//阻止触摸时浏览器的缩放、滚动条滚动
				        if(tos == 'top' || tos == 'bottom'){
					        init.ex = event.targetTouches[0].clientX;
					        var changeX = init.ex - init.sx;
					        if(Math.abs(changeX) > 5) {
					        	var x = tx = _tabs.offset().left;
			    	       		x = x + changeX;
			    	       		init.sx = init.ex;
				    	       	_this.scrollTabX(x);
					        }
				        }else if(tos == 'left' || tos == 'right'){
					        init.ey = event.targetTouches[0].clientY;
					        var changeY = init.ey - init.sy;
					        if(Math.abs(changeY) > 5) {
					        	var y = ty = _tabs.offset().top;
			    	       		y = y + changeY;
			    	       		init.sy = init.ey;
				    	       	_this.scrollTabY(y);
					        }
...
//				    _tabs[0].addEventListener("touchend",function() {
//				    }, false);
				},
				scrollTabX:function(x){
					if(x > 0){
						x = 0;
					}else{
						var w = _tabs.width() - _tabsOuter.width();
						if(Math.abs(x) > w){
							// 往左滑动，超过tabs总宽度减去屏幕宽度
							x = - w;
						}
					}
	    	        _tabs.css({'left':x+'px'});	
...
//					});
				...
				scrollTabY:function(y){
					if(y > 0){
						y = 0;
					}else{
						var h = _tabs.height() - _tabsOuter.height();
						if(Math.abs(y) > h){
							// 往上滑动，超过tabs总高度减去屏幕高度
							y = - h;
						}
					}
	    	        _tabs.css({'top':y+'px'});
...
//					_tabs.height(tsh);			
		...
		var defaluts = {
				...
				tabLayout:{				// 标签布局
					...
					tabsWidth:'match_parent',		// 标签栏宽度，只有设置居左或居右显示有效，默认为父容器宽度，可选项：自适应父层:match_parent,自适应自身:wrap_content
					scroll:true			// 允许滚动
				},
			 	scrollers : [],			// 滚动内容数组
		...
		var Tab = function(options){
			...
			var _icon = $('<img/>');
			var _label = $('<span class="label"></span>');
			var _this = null;
			return $.extend({
				create:function(){
					_this = this;
					...
					// 支持图标显示
					if(data.icon){
						var _span = $('<span class="icon"></span>');
				...
				setHover:function(){
					_tab.hover(function(){
						_tab.addClass('hover');
						if(_this.data.icon_hover && !_tab.hasClass('select')){
							_icon.attr('src',_this.data.icon_hover);
						}
					},function(){
						_tab.removeClass('hover');
						if(_this.data.icon && !_tab.hasClass('select')){
							_icon.attr('src',_this.data.icon);
						}
					});
				},
				select:function(){
					_tab.addClass('select');
					if(_this.data.iconSelect){
						_icon.attr('src',_this.data.iconSelect);
					}
				},			
				unSelect:function(){
