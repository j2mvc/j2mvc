...
public abstract class AuthInterceptor extends MeasureInterceptor{	
	...
	public Auth getAuthPath(String path){
		uri = getUri(request);
		Auth auth = authService.getAuthPath(path,AuthConfig.projectId);
		return auth;
	}
	/**
	 * 判断当前URI是路径还是链接
	...
	protected boolean auth(Auth auth){
		...
		if(!StringUtils.isEmpty(getUserId())){
			return authService.exists(auth.getId(), userId,AuthConfig.projectId);
		}
		return false;
	}
	/**
	...
	protected boolean auth(String value,int authType,String status){
		if(status.equals(AuthConstants.AUTH_STATUS_N)){
			// 关闭中的权限，权限不控制
			auth = authService.get(value,authType,status,AuthConfig.projectId);
			return auth!=null;
		}else{
			String userId = getUserId();
			...
			// 开启中的权限，需要判断用户是否拥有此权限
			if(!StringUtils.isEmpty(getUserId())){
				return authService.exists(value, authType,status,userId,AuthConfig.projectId);
			}
		}
		return false;
	}
