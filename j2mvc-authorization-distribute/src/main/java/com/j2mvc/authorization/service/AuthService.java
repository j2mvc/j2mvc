...
public class AuthService{
	...
	public int callSave(List<Auth> auths){
		...
		procedureSql.append("BEGIN\n"+
								...
								"SELECT COUNT(*) INTO count FROM "+tableName+" WHERE auth_type=a_type and auth_value=a_value and project_id=a_project_id;\n"+
								"IF count > 0 THEN\n"+
									(AuthConfig.enableUpdate? // 可自动更新
									"\tUPDATE "+tableName+" SET project_id=a_project_id,name=a_name,extra_limit=extra_limit,enable_edit=a_enable,tag=a_tag,tag=a_auth_none\n"+
									...
									:"") +
								"ELSE\n"+
									"\tINSERT INTO "+tableName+" (id,project_id,name,auth_value,auth_type,status,extra_limit,enable_edit,tag,auth_none) VALUES\n"+
	 ...
	 * 
	 */
	public Auth get(String value,int type,String status,String projectId) {
		String preSql = "SELECT * FROM " + tableName + " WHERE "
				+ "? regexp CONCAT('^',auth_value,'$') "
				+ "and auth_type=? and status=? and project_id=?";
		if(status.equals(AuthConstants.AUTH_TYPE_PATH) || status.equals(AuthConstants.AUTH_TYPE_URI)){
			// 查询字段值是否包含在URI中
			preSql = "SELECT * FROM " + tableName + " WHERE instr(?,auth_value) and auth_type=? and status=? and project_id=?";
		}
		Object object =  dao.get(preSql, new Object[]{value,type,status,projectId});
		return object!=null?(Auth)object:null;
	}
	/**
	 * 查找开启路径权限的权限
	 ...
	 * 
	 */
	public boolean exists(String id,String userId,String projectId) {
		String preSql = "SELECT COUNT(*) FROM " + tableName + " WHERE id=? and id in"
				...
				+ "(SELECT role_id FROM "+EntityConstants.TABLE_USER_ROLE+" WHERE user_id=?))"
						+ " and project_id=?";
		return dao.number(preSql, new Object[]{id,userId,projectId}) > 0;
	}
	 ...
	 * 
	 */
	public boolean exists(String value,int type,String status,String userId,String projectId) {
...
//		}
		preSql +=  " and auth_type=? and status=? and id in"
				...
				+ "(SELECT role_id FROM "+EntityConstants.TABLE_USER_ROLE+" WHERE user_id=?))"
						+ " and project_id=?";
		return dao.number(preSql, new Object[]{value,type,status,userId,projectId}) > 0;
	}
	/**
	 * 获取
	 ...
	 * 
	 */
	public boolean existsName(String name,String projectId) {
		String preSql = "SELECT count(*) FROM " + tableName + " WHERE name=? and project_id=?";
		return dao.number(preSql, new String[]{name,projectId})>0;
	}
	 ...
	 * 
	 */
	public boolean existsName(String name,String id,String projectId) {
		String preSql = "SELECT count(*) FROM " + tableName + " WHERE name=? and id<>? and project=?";
		return dao.number(preSql, new String[]{name,id,projectId})>0;
	}
	 ...
	 * 
	 */
	public boolean existsValue(String id,String value,int type,String projectId) {
		String preSql = "SELECT count(*) FROM " + tableName + " "
				+ "WHERE  id<>? "
				+ "and ? regexp  CONCAT('^',auth_value,'$')"
				+ " and auth_type=? and project_id=?";
		return dao.number(preSql, new Object[]{id,value,type,projectId})>0;
	}
}
